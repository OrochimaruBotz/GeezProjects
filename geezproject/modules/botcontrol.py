...
from geezproject.modules.sql_helper.bot_starters import (
    add_starter_to_db,
    get_all_starters,
    get_starter_details,
)
from geezproject.modules.sql_helper.globals import gvarstatus
from geezproject.utils import _format, asst_cmd, callback, reply_id
...
async def strone(event):
    ...
    async with event.client.conversation(pru) as conv:
        await conv.send_message(
            f"**Silahkan Kirimkan {var} Telethon anda dari @GeezStringBot**\n\nGunakan /cancel untuk membatalkan."
        )
        response = conv.wait_event(events.NewMessage(chats=pru))
        response = await response
        themssg = response.message.message
